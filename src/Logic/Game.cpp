...
void Game::Update(f32 i_deltaMs)
{
	...
	if (m_CameraComponent) {
		m_CameraComponent->Update(nullptr, i_deltaMs, m_CamKeyState);
	}
...
void Game::OnKey(const u32 i_keyCode, const bool i_isPressed)
{
	switch (i_keyCode) {
		case 0x26: // UP_ARROW
			{
				if (i_isPressed)
					SET_BIT(m_CamKeyState, 0x1);
				else CLEAR_BIT(m_CamKeyState, 0x1);
				break;
			}

		case 0x27: // RIGHT_ARROW
			{
				if (i_isPressed)
					SET_BIT(m_CamKeyState, 0x8);
				else CLEAR_BIT(m_CamKeyState, 0x8);
				break;
			}

		case 0x28: // DOWN_ARROW
			{
				if (i_isPressed)
					SET_BIT(m_CamKeyState, 0x2);
				else CLEAR_BIT(m_CamKeyState, 0x2);
				break;
			}

		case 0x25: // LEFT_ARROW
			{
				if (i_isPressed)
					SET_BIT(m_CamKeyState, 0x4);
				else CLEAR_BIT(m_CamKeyState, 0x4);
				break;
			}
		default:
			break;

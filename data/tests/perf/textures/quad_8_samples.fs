...
void main()
{
	...
	mediump vec3 c4 = texture(u_MainTex, vec2(v_TexCoord.x - iu_TexelSize.x, v_TexCoord.y + iu_TexelSize.y)).rgb;
	mediump vec3 c5 = texture(u_MainTex, vec2(v_TexCoord.x + iu_TexelSize.x, v_TexCoord.y - iu_TexelSize.y)).rgb;
	mediump vec3 c6 = texture(u_MainTex, vec2(v_TexCoord.x + iu_TexelSize.x, v_TexCoord.y - iu_TexelSize.y)).rgb;
	mediump vec3 c7 = texture(u_MainTex, vec2(v_TexCoord.x + iu_TexelSize.y, v_TexCoord.y + iu_TexelSize.y)).rgb;
	mediump vec3 col = mainColor + c0 + c1 + c2 + c3 + c4 + c5 + c6 + c7;
	col /= 9.0f;

...
void main()
{
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        highp vec3 H  = ImportanceSampleGGX(Xi, N, iu_roughness);
        ...
        if(NdotL > 0.0)
        {
			...
			highp float D   = DistributionGGX(N, H, iu_roughness);
			highp float NdotH = max(dot(N, H), 0.0);
			...
			highp float mipLevel = iu_roughness == 0.0 ? 0.0 : 0.5 * log2(saSample / saTexel);

			highp vec3 hdrColor = textureLod(u_EnvMap, L, mipLevel).rgb;
